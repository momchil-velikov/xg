...
/* Output type.  */
enum output_type { output_defines = 1, output_slr, output_lalr,
                   output_random_sentence };
...
print_version ()
{
  ...
  fputs ("This is free software; see the source for copying conditions."
         "  There is NO\n", stderr);
  fputs ("warranty; not even for MERCHANTABILITY or FITNESS FOR A PARTICULAR"
         " PURPOSE.\n", stderr);
  return -1;
}
...
{
  if (xg_input)
    fprintf (stderr, "xg: WARNING: input file ``%s'' overrides ``%s''\n",
             arg, xg_input);
  xg_input = arg;
  return 0;
}
...
static ulib_option options [] =
{
  { .key = 'V', .name = "version", .cb = print_version,
    .help = "\toutput version information and exit" },

  { .key = 'h', .name = "help", .cb = print_usage,
    .help = "\t\tdisplay this help and exit" },

  { .key = 't', .name = "debug", .flag = &xg_flag_debug, .value = 1,
    .help = "\t\tinstrument the parser for debugging (TODO)" },

  { .key = 'n', .name = "namespace", .cb = handle_namespace,
    .flags = ulib_option_required_arg, .arg = "<name>",
    .help = "\n\t\t\tset the namespace for the external symbols (TODO)" },

  { .key = 'o', .name = "output", .cb = handle_output,
    .flags = ulib_option_required_arg, .arg = "<path>",
    .help = "\n\t\t\tname output file" },

  { .key = 'D', .name = "defines", .flag = &xg_flag_output_type,
    .value = output_defines,
    .help = "\toutput a header file with token #defines (TODO)" },

  { .key = 'S', .name = "slr", .flag = &xg_flag_output_type,
    .value = output_slr,
    .help = "\t\toutput an SLR(1) parser" },

  { .key = 'L', .name = "lalr", .flag = &xg_flag_output_type,
    .value = output_lalr,
    .help = "\t\toutput a LALR(1) parser" },

  { .key = 's', .name = "sentence", .flag = &xg_flag_output_type,
    .value = output_random_sentence,
    .help = "\tgenerate random sentence" },

  { .key = 'z', .name = "sentence-size", .cb = handle_sentence_size,
  ...
  if (xg_flag_output_type == output_slr || xg_flag_output_type == output_lalr)
    {
      ...
      if ((xg_flag_output_type == output_slr
           && xg_make_slr_reductions (g, dfa) < 0)

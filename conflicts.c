...
static void
log_shift_reduce_conflict(unsigned int state,
                          const xg_symdef *la,
                          const xg_lr0reduct *rd,
                          enum xg_resolution res) {
    const char *name;
    char nm[2];
    ...
    ulib_log_printf(
        xg_log, "State %u: shift/reduce conflict between symbol ``%s''", state, name);
    ...
    ulib_log_printf(xg_log,
                    ...
                    res == xg_resolve_shift || res == xg_resolve_shift_default
                        ? "shift"
                        : res == xg_resolve_reduce ? "reduce" : "error");
}

/* Log a reduce/reduce conflict resolution.  */

static void
log_reduce_reduce_conflict(unsigned int state,
                           const xg_symdef *la,
                           unsigned int p1,
                           unsigned int p2,
                           unsigned int res) {
...
static void
resolve_shift_reduce_conflicts(const xg_grammar *g, xg_lr0dfa *dfa, xg_lr0state *state) {
    unsigned int trno;
    ...
    while (trno < xg_lr0state_trans_count(state)) {
        ...
        if (r == xg_resolve_none || r == xg_resolve_shift
...
resolve_reduce_reduce_conflicts(const xg_grammar *g, xg_lr0state *state) {
    ...
    for (i = 0; i < n; ++i) {
        ...
        for (j = i + 1; j < n; ++j) {
            ...
            for (b = 0; b < bmax; ++b) {
                if (ulib_bitset_is_set(&rdi->la, b) && ulib_bitset_is_set(&rdj->la, b)) {
                    def = xg_grammar_get_symbol(g, b);
                    if (rdi->prod < rdj->prod) {
                        ulib_bitset_clear(&rdj->la, b);
                        log_reduce_reduce_conflict(
                            state->id, def, rdi->prod, rdj->prod, rdi->prod);
                    } else {
                        ulib_bitset_clear(&rdi->la, b);
                        log_reduce_reduce_conflict(
                            state->id, def, rdi->prod, rdj->prod, rdj->prod);
                    }
                }
            }
        }

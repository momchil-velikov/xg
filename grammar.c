...
static int
init_caches(void) {
    symdef_cache = ulib_cache_create(ULIB_CACHE_SIZE,
                                     sizeof(xg_symdef),
                                     ULIB_CACHE_ALIGN,
                                     sizeof(unsigned int),
                                     ULIB_CACHE_CTOR,
                                     ...
                                     0);
    if (symdef_cache != 0) {
        prod_cache = ulib_cache_create(ULIB_CACHE_SIZE,
                                       sizeof(xg_prod),
                                       ULIB_CACHE_ALIGN,
                                       sizeof(void *),
                                       ULIB_CACHE_CTOR,
        ...
        if (prod_cache != 0)
            ...
            ulib_log_printf(xg_log, "ERROR: Unable to create the production cache");
    } else
        ...
        fprintf(out,
                ...
                (def->assoc == xg_assoc_unknown
                     ...
                     : def->assoc == xg_assoc_none
                           ? "none"
                           : def->assoc == xg_assoc_left ? "left" : "right"),
                def->prec);
    else
        fputs("]:\n", out);

...
#define errorv(ctx, fmt, ...) \
    ulib_log_printf(xg_log, "%s:%d: ERROR: " fmt, ctx->name, ctx->lineno, __VA_ARGS__)
    ...
    if (ctx->token == TOKEN_PREC) {
        ...
        if (ctx->token == TOKEN_WORD) {
            ...
            if (def->assoc == xg_assoc_unknown) {
                errorv(ctx, "Unknown precedence and associativity of ``s''", def->name);
                return -1;
            }
            ...
            if (def->assoc == xg_assoc_unknown) {
                errorv(ctx, "Unknown precedence and associativity of '%c'", def->code);
                return -1;
            }
        } else {
...
parse_rhs(parse_ctx *ctx, xg_symdef *lhs) {
    ...
    while (ctx->token == '|') {
        ...
        if (getlex(ctx) < 0 || parse_rhs_alternative(ctx, lhs) < 0)
            return -1;
    }
...
xg_grammar_read(const char *name) {
    ...
    if ((sym = xg_symdef_new_copy("<start>")) == 0)
        goto error;
    ...
    start->lhs = sym->code;
    if (xg_prod_add(start, ctx.gram->start) < 0 || xg_prod_add(start, XG_EOF) < 0
